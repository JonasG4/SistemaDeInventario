...
import { getAllCategorias } from "../../store/categorias";
import { InputSearchWithFilter } from "../shared/forms/FormInputs";
import AddCategoria from "./modales/registroModal";
import DeleteCategoria from "./modales/eliminarModal";
import UpdateCategoria from "./modales/editarModal";
    ...
    <div className="min-w-full flex flex-col">
            ...
            {Object.values(categorias).map((categoria, index) => (
              <tr className="odd:bg-slate-100" key={index}>
                ...
                <td className="px-6 py-4 flex items-center justify-center gap-2">
                  <UpdateCategoria id={categoria.id_categoria} />
                  <DeleteCategoria id={categoria.id_categoria} />
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex pt-5 items-center justify-between w-full">
        <p className="text-slate-600 font-bold text-md">
          {categorias.length}
          {categorias.length > 1 ? " elementos" : " elemento"}
        </p>
        <div>
